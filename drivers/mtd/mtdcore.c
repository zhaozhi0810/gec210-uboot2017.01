...
/*2020-12-22*/
        ...
        if ((to == 0) )
        {
            if( (buf[1] != 0x20) && (buf[1] != 0x40))
            {
                printf("nand_write error: no header info(16bytes).\n");
                return 1;
            }
            else
            {               
                ...
                {
                    j = buf[0] | (buf[1]<<8);
                    printf("calculated j =0x %x\n",j);
                    ptr = buf + 16;
                    for(i = 16, checksum = 0; i < j; i++) {
                        checksum += *ptr;
                        ptr++;
                    }
                    *((volatile u32 *)(buf + 0x8)) = checksum;
                    printf("Checksum is calculated =0x %x.\n",*(( u32 *)(buf + 0x8)) );
                }
            }
        }
#endif    
